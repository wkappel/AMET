...
#     my.leaf <- my.leaf.base
        for (j in 1:length(network_names)) {
           if(i == 1) { 
	      plot_val <- sinfo_data[[j]]$Obs_Val
	   }
           if(i == 2) { 
	      plot_val <- sinfo_data[[j]]$Mod_Val 
	   }
           if(i == 3) { 
	      plot_val <- sinfo_data[[j]]$Diff_Val
	   }
	   layer_id <- rep(i,each=length(plot_val))
...
#           my.leaf <- my.leaf.base
           my.leaf <- my.leaf %>% addCircleMarkers(sinfo_data[[j]]$lon,sinfo_data[[j]]$lat,color="black",fillColor=~binpal2(plot_val),group=network_names[[j]],layerId=~paste("OBS",sinfo_data[[j]]$stat_id,sep=""),layer_id,radius=plot_rad*symbsizfac,data=data.df,opacity=1,fillOpacity=fill_opacity,stroke=TRUE,weight=1,popup=contents,label=contents2, labelOptions = labelOptions(noHide = F, textsize = "15px")) 
         #} 
           ...
           my.leaf <- my.leaf %>%
          addLayersControl(
            baseGroups = plot_names, overlayGroups = c(network_names), options =  layersControlOptions(collapsed = FALSE,position="topleft")
          ) %>%
   ...
   saveWidget(my.leaf, file=filename[1],selfcontained=T)
