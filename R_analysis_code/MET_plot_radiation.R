...
if(spatial) {
  ...
  for(s in 1:nsites) {
    ...
    writeLines(paste("Computing spatial statistics for site:",sites[s],s,"of",
                      nsites," --- Centering array around solar noon ~",utc_max))
    for(hh in 1:nhrbin) {
  ...
  if (textstats){   
    spatialRdf<- paste(figdir,"/srad.spatial",".",d1,"-",d2,".Rdata",sep="")
    save(stat_array, file=spatialRdf)
  }
  #-------------------------------------------------------------------------------  
 ...
 for(ss in 1:nsites){
  ...
  for(hh in 1:24) {     
    ...
    if(hits > 0) {
      ...
      # QC check of BSRN. Where average model is 0 (night), set BSRN to 0 and stats NA
      if(stat_array_diurnal[2,hh] == 0.0) {
        stat_array_diurnal[1,hh]<- 0.0
        stat_array_diurnal[3:7,hh]<-NA
      }
    }
    sample.count <- sum(ifelse(mask_obs,1,0)) + sample.count 
    writeLines(paste("Site",sites[ss],"has",sum(ifelse(mask_obs,1,0)),"days of radiation obs at",hour_obs_utc[hh],"UTC"))     
  ...
  if(sample.count < 24) {
    writeLines(paste("Site",sites[ss]," has fewer than 24 total samples considering all hours.",
                     "Abort evaluation and move to next site."))
    next
  }
