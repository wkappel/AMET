...
boxplot(split(aqdat.df$ob_val, aqdat.df$hour), range=0, border=plot_colors[1], whiskcol=whisker_color[1], staplecol=whisker_color[1], col=plot_colors[1], ylim=c(y.axis.min, y.axis.max), xlab=paste("Hour (",TIME_FORMAT,")"), ylab=label, cex.axis=1.3, cex.lab=1.3)	# Create boxplot
