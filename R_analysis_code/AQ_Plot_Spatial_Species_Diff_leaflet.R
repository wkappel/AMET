...
filename[1]	<- paste(run_name1,species[1],pid,"spatialplot_obs.html",sep="_")           # Filename for obs spatial plot
filename[2]	<- paste(run_name1,species[1],pid,"spatialplot_mod.html",sep="_")           # Filename for model spatial plot
filename[3]	<- paste(run_name1,species[1],pid,"spatialplot_diff.html",sep="_")          # Filename for diff spatial plot
filename_png[1] <- paste(run_name1,species[1],pid,"spatialplot_obs.png",sep="_")           # Filename for obs spatial plot
filename_png[2] <- paste(run_name1,species[1],pid,"spatialplot_mod.png",sep="_")           # Filename for model spatial plot
filename_png[3] <- paste(run_name1,species[1],pid,"spatialplot_diff.png",sep="_")          # Filename for diff spatial plot
filename_rat	<- paste(run_name1,species[1],pid,"spatialplot_ratio",sep="_")         # Filename for diff spatial plot
...
for (j in 1:total_networks) {							# Loop through for each network
   ...
   #########################
   {
      if (Sys.getenv("AMET_DB") == 'F') {
         sitex_info       <- read_sitex(Sys.getenv("OUTDIR"),network,run_name1,species_in[1])
         ...
         query_result   <- query_dbase(run_name1,network,species_in[1])
   ...
   {
      if (Sys.getenv("AMET_DB") == 'F') {
         sitex_info       <- read_sitex(Sys.getenv("OUTDIR"),network,run_name1,species_in[2])
         data_exists      <- sitex_info$data_exists
         if (data_exists == "y") {
         ...
         query_result2   <- query_dbase(run_name1,network,species_in[2])
         aqdat_query2.df <- query_result2[[1]]
         data_exists    <- query_result2[[2]]
         if (data_exists == "y") { units <- query_result2[[3]] }
      }
   }
   #######################
   ob_col_name <- paste(species_in[1],"_ob",sep="")
   mod_col_name <- paste(species_in[1],"_mod",sep="")
   mod_col_name2 <- paste(species_in[2],"_mod",sep="")
   { 
      if (data_exists == "n") {
	 ...
	 aqdat_in2.df <- data.frame(Network=I(aqdat_query.df$network),Stat_ID=I(aqdat_query.df$stat_id),lat=aqdat_query.df$lat,lon=aqdat_query.df$lon,State=aqdat_query.df$state,Obs_Value=round(aqdat_query.df[[ob_col_name]],5),Mod_Value=round(aqdat_query2.df[[mod_col_name2]],5),Hour=aqdat_query.df$ob_hour,Start_Date=aqdat_query.df$ob_dates,Month=aqdat_query.df$month)
	 ...
	 aqdat2.df <- Average(aqdat_in2.df)
         Mod_Obs_Diff <- aqdat.df$Mod_Value-aqdat2.df$Mod_Value
         Mod_Obs_Rat  <- aqdat.df$Mod_Value/aqdat2.df$Mod_Value
         ...
         ##################################################
         sinfo_data[[k]]<-list(stat_id=aqdat.df$Stat_ID,lat=aqdat.df$lat,lon=aqdat.df$lon,State=aqdat.df$State,Obs_Val=aqdat.df$Obs_Value,Mod_Val=aqdat.df$Mod_Value,Diff_Val=aqdat.df$Mod_Obs_Diff,Network=network)
...

{
   if (max(quantile(data_all,probs=quantile_max)) < 1) {
      data.seq1 <- pretty(seq(floor(min(quantile(data_all,probs=quantile_min))),ceiling(max(quantile(data_all,probs=quantile_max),na.rm=T))),n=20)
   }
   else {
...
for (i in 1:3) {
   if (custom_title == "") {
      if (i == 1) { plot_title <- paste(run_name1,species_in[1],"Ob Values",dates,sep=" ") }
      if (i == 2) { plot_title <- paste(run_name1,species_in[1],"Model Values",dates,sep=" ") }
      if (i == 3) { plot_title <- paste(run_name1,species_in[1],"-",species_in[2],dates,sep=" ") }
   }
   ...
   my.leaf <- my.leaf %>% addLegend("bottomright", pal = binpal2, values = c(min.data,max.data), title = paste(species_in[1],"<br/> (",units,")",sep=""), opacity = 2) 
   ...
   my.leaf <- my.leaf %>% addLayersControl(baseGroups = base_Groups, overlayGroups = c(network_names), options =  layersControlOptions(collapsed = FALSE,position="topleft"))
