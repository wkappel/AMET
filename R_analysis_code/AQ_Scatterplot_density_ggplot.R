...
sp <- ggplot(aqdat.df,aes(x=Obs_Value,y=Mod_Value)) + geom_hex(bins=100) + scale_fill_gradientn(colours=c("light blue","blue","dark green","yellow","orange","red")) + geom_abline(intercept = 0, slope=1) + xlim(axis.min,axis.max) + ylim(axis.min,axis.max) + geom_smooth(method=lm, linetype="dashed", color="black") + labs(title=main.title,x=network,y=model_name) + scale_y_continuous(expand=c(0,0), limits=c(axis.min,axis.max), breaks = pretty(c(axis.min,axis.max), n = 10)) + scale_x_continuous(expand=c(0,0), limits=c(axis.min,axis.max), breaks = pretty(c(axis.min,axis.max), n = 10)) + theme(legend.justification=c(1,0), legend.position=c(0.98,0.02), legend.background=element_blank(), legend.key=element_blank(), plot.title=element_text(hjust=0.5, size=10))
...
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("Y =",signif(y.x.lm[1],2),"+",signif(y.x.lm[2],2),"* X"),x=x[2],y=y9,showarrow=FALSE))
p <- p %>% layout(annotations=list(text=paste("(",units,")                          (%)",sep=""),x=x[3],y=y10,showarrow=FALSE,align="left"))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("r = ",sprintf("%.2f",corr),"                    NMB = ",sprintf("%.1f",nmb)),x=x[2],y=y11,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("RMSE = ",sprintf("%.2f",rmse),"             NME = ",sprintf("%.1f",nme)),x=x[2],y=y12,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("RMSE[s] = ",sprintf("%.2f",rmse_sys),"        NMdnB = ",sprintf("%.1f",nmdnb)),x=x[2],y=y13,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("RMSE[u] = ",sprintf("%.2f",rmse_unsys),"        NMdnE = ",sprintf("%.1f",nmdne)),x=x[2],y=y14,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("MB = ",sprintf("%.2f",mb),"                FB = ",sprintf("%.1f",fb)),x=x[2],y=y15,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("ME = ",sprintf("%.2f",me),"                  FE = ",sprintf("%.1f",fe)),x=x[2],y=y16,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("MdnB = ",sprintf("%.2f",med_bias)),x=x[2],y=y17,showarrow=FALSE))
p <- p %>% layout(annotations=list(xanchor="left",align="left",text=paste("MdnE = ",sprintf("%.2f",med_error)),x=x[2],y=y18,showarrow=FALSE))
