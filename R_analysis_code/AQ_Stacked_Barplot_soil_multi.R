...
if (total_networks > 1) {
   ...
   num_sites2	<- length(unique(aqdat_sub2.df$stat_id)) 
   num_pairs2	<- length(aqdat_sub2.df$stat_id)
}
...
medians_tot_ob  <- medians.df$Cl_ob+medians.df$Na_ob+medians.df$Fe_ob+medians.df$Al_ob+medians.df$Ti_ob+medians.df$Si_ob+medians.df$Ca_ob+medians.df$Mg_ob+medians.df$K_ob
...
if (total_networks > 1) {
    ...
    medians_tot_ob2	<- medians2.df$Cl_ob+medians2.df$Na_ob+medians2.df$Fe_ob+medians2.df$Al_ob+medians2.df$Ti_ob+medians2.df$Si_ob+medians2.df$Ca_ob+medians2.df$Mg_ob+medians2.df$K_ob
...
if (total_networks > 1) {
   data_matrix <- matrix(c(medians.df$Fe_ob,medians.df$Fe_mod,medians2.df$Fe_ob,medians2.df$Fe_mod,medians.df$Al_ob,medians.df$Al_mod,medians2.df$Al_ob,medians2.df$Al_mod,medians.df$Ti_ob,medians.df$Ti_mod,medians2.df$Ti_ob,medians2.df$Ti_mod,medians.df$Si_ob,medians.df$Si_mod,medians2.df$Si_ob,medians2.df$Si_mod,medians.df$Ca_ob,medians.df$Ca_mod,medians2.df$Ca_ob,medians2.df$Ca_mod,medians.df$Mg_ob,medians.df$Mg_mod,medians2.df$Mg_ob,medians2.df$Mg_mod,medians.df$K_ob,medians.df$K_mod,medians2.df$K_ob,medians2.df$K_mod,medians.df$Mn_ob,medians.df$Mn_mod,medians2.df$Mn_ob,medians2.df$Mn_mod,medians.df$Cl_ob,medians.df$Cl_mod,medians2.df$Cl_ob,medians2.df$Cl_mod,medians.df$Na_ob,medians.df$Na_mod,medians2.df$Na_ob,medians2.df$Na_mod),byrow=T,ncol=4)  # create matrix of average values needed by barplot, 2 columns (ob,model) and species by row
yaxis.max   <- (max(c(sum(data_matrix[,1]),sum(data_matrix[,2]),sum(data_matrix[,3]),sum(data_matrix[,4]))))*1.1        # find the max of the average values to set y-axis max
}
...
{
   if (total_networks > 1) {
      simulations	<- paste(network_label[1],run_name1,network_label[2],run_name1,sep=",")
      sim_names		<- c(network_label[1],model_name,network_label[2],model_name)
   }
   else {
...
if (run_info_text == "y") {
   if (rpo != "None") {
      text(2.07,yaxis.max*.6,paste("RPO: ",rpo,sep=""),cex=1,adj=c(1,0))
   }
   if (pca != "None") {
      text(2.07,yaxis.max*.6,paste("PCA: ",pca,sep=""),cex=1,adj=c(1,0))
   }
   if (site != "All") {
      text(2.07,yaxis.max*.56,paste("Site: ",site,sep=""),cex=1,adj=c(1,0))
   }
   if (state != "All") {
      text(2.07,yaxis.max*.53,paste("State: ",state,sep=""),cex=1,adj=c(1,0))
   }
} 
############################################################
...
###################################
text(2.07,yaxis.max*.47,paste("# of sites:",num_sites,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.44,paste("# of obs: ",num_pairs,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.41,paste("RMSE: ",rmse,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.38,paste("RMSEs: ",rmse_sys,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.35,paste("RMSEu: ",rmse_unsys,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.32,paste("IA:   ",index_agree,sep=""),cex=1,adj=c(1,0))
text(2.07,yaxis.max*.29,paste("r:   ",correlation,sep=""),cex=1,adj=c(1,0))

if (total_networks > 1) {
   ### Add statistics to plot area ###
   text(0,yaxis.max*0.9,paste("Simulation:",run_name1),cex=1,adj=c(0,0))
   text(2.07,yaxis.max*.50,network_names[1],cex=1,adj=c(1,0))
   text(2.07,yaxis.max*.22,network_names[2],cex=1,adj=c(1,0))
   text(2.07,yaxis.max*.19,paste("# of sites ",num_sites2,sep=""),cex=1,adj=c(1,0))
   text(2.07,yaxis.max*.16,paste("# of obs ",num_pairs2,sep=""),cex=1,adj=c(1,0))
