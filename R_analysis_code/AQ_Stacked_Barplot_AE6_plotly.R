header <- "
########################## AE6 STACKED BAR PLOT (PLOTLY) ##############################
### AMET CODE: AQ_Stacked_Barplot_AE6_plotly.R
###
### This code creates an interactive stacked bar plot of PM species from the IMPROVE, CSN,
### SEARCH or AQS Daily networks using the plotly R package. Data are then averaged for SO4,
### NO3, NH4, EC, OC soil species, NCOM, PMother and total PM2.5. These averages are then
...
for (j in 1:length(run_names)) {
   ...
   {
      if (Sys.getenv("AMET_DB") == 'F') { aqdat_all.df <- aqdat_all.df[ -c(1,3,4,6,7,8,9,44)] }
      else { aqdat_all.df     <- aqdat_all.df[ -c(1,3,4,5,6,8,9,10,45,46,47)] }
   }
...
p <- plot_ly(data=merged.df,x=ob_mod,y=~SO4,type="bar",height=img_height,width=img_width,name="SO4",marker=list(color="red"),text=paste("% of total:",SO4_perc,"<br># of sites:",NUM_SITES,"<br># of pairs:",NUM_PAIRS)) %>%
  ...
  layout(title=main.title,yaxis=list(title=paste(method," Concentration (",units,")")),barmode='stack', xaxis=xform)
