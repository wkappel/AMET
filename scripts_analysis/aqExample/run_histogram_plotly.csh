...
# distribution. The script can only accept a single network and
# single simulation at this time. The code can be modified to add
# another simulation however.
#
# This script was added to AMET as part of the AMETv1.3 code.
#
...
##--------------------------------------------------------------------------##
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_histogram_bias.html"
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
