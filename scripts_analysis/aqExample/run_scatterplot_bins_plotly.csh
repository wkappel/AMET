...
# only a single network and species. This version of the script uses the
# plotly package to create an interactive html file.
# 
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/scatterplot_bins_plotly
...
#> Non-standard networks (should probably be set to F unless specifically required)
  ...
  setenv AMET_LOG scatterplot_bins_plotly.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Scatterplot_bins_plotly.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_scatterplot_bins.$AMET_PTYPE"
		echo "Data File -------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_scatterplot_bins.csv"
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
