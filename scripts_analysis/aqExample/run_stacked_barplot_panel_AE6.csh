...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/stacked_barplot_panel_AE6
...
#> Non-standard networks (should probably be set to F unless specifically required)
  ...
  setenv AMET_LOG stacked_barplot_panel_AE6.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Stacked_Barplot_panel_AE6.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots -- --------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_PM2.5_stacked_barplot_panel_AE6.$AMET_PTYPE
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
