...
# Purpose:
#
# This is a custom boxplot which is designed specifically for comparing
# multiple simulation, although it works with single simulation. Box plots
# of the observation and corresponding model simulation(s) are plotted, 
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/boxplot_roselle
...
#> Non-standard networks (should probably be set to F unless specifically required)
  ...
  setenv AMET_LOG boxplot_roselle.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Boxplot_Roselle.R $AMET_LOG 
  ...
  echo
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plot   ---------->" $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_boxplot_roselle.$AMET_PTYPE
		echo "-----------------------------------------------------------------------------------------"
		exit(0)
  else
