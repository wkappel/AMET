#!/bin/csh -f
# -----------------------------------------------------------------------
# AERO6 and beyond stacked bar plot timeseries
# -----------------------------------------------------------------------
# Purpose:
#
...
# IMPROVE networks.  Data are then daily averaged for SO4, NO3, NH4, EC, OC
...
# soil species,  NaCl, and NCOM are not available before AERO6. The script
# will work for CMAQ aerosol modules 6 and beyond. This script produces pdf and
# png plots using the ggplot2 package and an interactive html plot using plotly.
#
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/stacked_barplot_AE6_ts
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Stacked_Barplot_AE6_ts.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
                echo "Plots -- --------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_stacked_barplot_AE6_ts.pdf
                echo "Plots -- --------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_stacked_barplot_AE6_ts.png
		echo "Plots -- --------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_stacked_barplot_AE6_ts.html
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
