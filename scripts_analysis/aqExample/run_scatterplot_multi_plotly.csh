...
# plot. Only one network and species can be plotted at a time. Additional
# projects beyond two can be specified in the corresponding input file
# for this script. The output from this file is an interactive html file
# produced using the R plotly package.
#
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/scatterplot_multi_plotly
...
#> Non-standard networks (should probably be set to F unless specifically required)
  ...
  setenv AMET_LOG scatterplot_multisim_plotly.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Scatterplot_multisim_plotly.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "Plots ----------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_scatterplot_multi.$AMET_PTYPE
                echo "Data File ------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_${AMET_PID}_scatterplot_multi_data.csv
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
