#!/bin/csh -f
# -----------------------------------------------------------------------
# Spatial plot - Model to Model Speices Ratio
# -----------------------------------------------------------------------
# Purpose:
#
# This code is part of the AMET-AQ system.  The Plot Spatial MTOM Species 
# code takes a MYSQL database query for two species from one or more
# networks and two model simulations and plots the minimum, maximum,
# average difference, and ratio between two species from two model simulations
# at the network sites. No observation data are used in the calculation, but 
# at least one observation network must be specified.
#
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/plot_spatial_mtom_species
  ...
  setenv AMET_AQSPECIES SO4,PM_TOT
...
#> Non-standard networks (should probably be set to F unless specifically required)
  ...
  setenv AMET_LOG plot_spatial_mtom_species.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Plot_Spatial_MtoM_Species.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots -- ---------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_spatialplot_mtom_species_diff_avg.$AMET_PTYPE"
                echo "Plots -- ---------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_spatialplot_mtom_species_diff_min.$AMET_PTYPE"
                echo "Plots -- ---------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_spatialplot_mtom_species_diff_max.$AMET_PTYPE"
                echo "Plots -- ---------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_spatialplot_mtom_species_diff_perc.$AMET_PTYPE"
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
