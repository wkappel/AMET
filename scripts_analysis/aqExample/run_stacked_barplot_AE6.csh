...
# the percent of the total PM2.5 that each species comprises. This script
...
# will work for CMAQ aerosol modules 6 and beyond.
#
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Stacked_Barplot_AE6.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "Plots ----------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_stacked_barplot_AE6.$AMET_PTYPE
                echo "Data File ------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_PID}_stacked_barplot_AE6_data.csv
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
