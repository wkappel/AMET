...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/summary_panel_plot
...
#> Met networks
  ...
  setenv AMET_LOG summary_panel_plot.log
...
setenv AMETRINPUT $AMETBASE/scripts_analysis/$AMET_PROJECT/input_files/all_scripts.input
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Summary_Panel_Plot.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots ----------------------->" $AMET_OUT/${AMET_PROJECT}_${AMET_METSPECIES}_${AMET_PID}_summary_panel_plot.$AMET_PTYPE
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
