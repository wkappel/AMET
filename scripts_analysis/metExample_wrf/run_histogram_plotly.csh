...
# Purpose:
#
# This script creates one histogram plot containing the  modeled and observed 
# data and the bias distribution. The script can only accept a single network 
# and single simulation at this time. The code can be modified to add
# another simulation however.
#
# This script was added to AMET as part of the AMETv1.6 code.
#
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/histogram_plotly
...
#> Met networks
  ...
  setenv AMET_LOG histogram_plotly.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Histogram_plotly.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_METSPECIES}_${AMET_PID}_histogram.html"
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
