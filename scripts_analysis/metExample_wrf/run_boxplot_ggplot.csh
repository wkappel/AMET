#!/bin/csh -f
# --------------------------------
# Boxplot using ggplot R package
# -----------------------------------------------------------------------
# Purpose:
#
...
# The temporal averaging in the script is controlled using the averageing
# flag in the all_scripts.input file. The output from this script is are 
# pdf and png files created using the ggplot2 package in R.
#
...
# -----------------------------------------------------------------------
  ...
  setenv AMET_OUT       $AMETBASE/output/$AMET_PROJECT/boxplot_ggplot
...
#> Met networks
  ...
  setenv AMET_LOG boxplot_ggplot.log
...
##--------------------------------------------------------------------------##
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R_analysis_code/AQ_Boxplot_ggplot.R $AMET_LOG 
  ...
  echo
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plot   ---------->" $AMET_OUT/${AMET_PROJECT}_${AMET_METSPECIES}_${AMET_PID}_boxplot_ggplot.$AMET_PTYPE
                echo "Plot   ---------->" $AMET_OUT/${AMET_PROJECT}_${AMET_METSPECIES}_${AMET_PID}_boxplot_bias_ggplot.$AMET_PTYPE
		echo "Data File ------->" $AMET_OUT/${AMET_PROJECT}_${AMET_METSPECIES}_${AMET_PID}_boxplot_ggplot_data.csv
		echo "-----------------------------------------------------------------------------------------"
		exit(0)
  else
